...
public class FlappyBird extends Application {
    ...
    private int lives = 0;
    private boolean playWithLives = true;
    ...
    public void start(Stage primaryStage) {
        ...
        //added button to allow user to play with lives
        Button playWithLives = new Button("Play with lives");
        playWithLives.setOnAction( e -> {
            this.playWithLives = !this.playWithLives;
            if(this.playWithLives){
                playWithLives.setText("Play without lives")
            }
            else {
                playWithLives.setText("Play with lives")
            }
        });
        root.getChildren().add(playWithLives);

        //represents number of lives player has
        //stays at 0 if player isn't playing with lives
        Label numberOfLives = new Label("0");
        numberOfLives.setFont(10);
        root.getChildren().add(numberOfLives);
        ...
        for (Pipe pipe : pipes) {
            ...
            if (!pipe.isPassed() && bird.getCircle().getCenterX() > pipe.getX() + pipe.getWidth()) {
                ...
                if (score > highScore) {
                    ...
                    if (playWithLives) {
                        lives++;
                        numberOfLives.setText(String.valueOf(lives));
                    }
